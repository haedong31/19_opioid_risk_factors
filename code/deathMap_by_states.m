...
% find the maximum number of deaths for within the entire data sets
max_deaths = max(kff_data_maxs);
min_deaths = min(kff_data_mins);
...
for i = 1:numel(file_paths)
    ...
    colormap(jet)
    colorbar
    txt = ['Opioid Overdoes Deaths in the U.S. ' year];
    title(txt);
    ...
    faceColors = makesymbolspec('Polygon', ...
        {'Deaths', [min_deaths max_deaths], 'FaceColor', jet}, ...
...
%% Video recording
video_obj = VideoWriter('deaths_state_abs.mp4', 'MPEG-4');
